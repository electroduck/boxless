...
int main(int argc, char* argv[]) {
	HDESK hHiddenDesktop = NULL;
	std::stringstream ssDesktopName;
	bool bDeskSet = false;

	try {
		// Move us onto another desktop
		ssDesktopName << "DOSDesktop" << GetCurrentProcessId();
		hHiddenDesktop = CreateDesktopA(ssDesktopName.str().c_str(), NULL, NULL, 0, GENERIC_ALL, NULL);
		if (!hHiddenDesktop) throw std::runtime_error("Error creating desktop");
		if (!SetThreadDesktop(hHiddenDesktop)) throw std::runtime_error("Error setting thread desktop");
		bDeskSet = true;

		// Prevent use of DirectX video driver, which uses stinky DirectInput
		if (putenv("SDL_VIDEODRIVER=windib") < 0)
			throw std::runtime_error("Error setting video driver");
...
#ifndef DISABLE_JOYSTICK // This must be defined to ensure we don't use DirectShitput
	...
	sdl.raltstate = SDL_KEYUP;
		...
		if (SDL_VideoDriverName(sdl_drv_name,128)!=NULL) {
			if (strcmp(sdl_drv_name,"windib")==0) LOG_MSG("SDL_Init: Starting up with SDL windib video driver. Excellent!");
		}
#endif
...
// Fix for SDLMain breaking as it always does
