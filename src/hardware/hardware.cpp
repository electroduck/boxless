...
// Never-before seen: actual use of C++ in this "C++" program
#include <sstream>
#include <iomanip>
...
typedef void(__stdcall* AcceptFrame_t)(unsigned nWidth, unsigned nHeight, unsigned nBPP, unsigned nPitch, unsigned flags,
	float fFPS, unsigned char* pData, unsigned char* pPalette);
typedef void(__stdcall* HookInit_t)(void);

void CAPTURE_AddImage(Bitu width, Bitu height, Bitu bpp, Bitu pitch, Bitu flags, float fps, Bit8u * data, Bit8u * pal) {
	// This is our hook into the morass that is the DOSBox code

	// Starts out as INVALID_HANDLE_VALUE. If it is loaded OK, it changes to a valid handle, else it changes to NULL.
	static HMODULE hModHook = (HMODULE)INVALID_HANDLE_VALUE;
	static AcceptFrame_t procAcceptFrame = nullptr;
	static HookInit_t procHookInit = nullptr;

	if (hModHook == INVALID_HANDLE_VALUE) {
		hModHook = LoadLibraryA("hookbox.dll");

		if (hModHook == NULL) {
			DWORD dwError = GetLastError();
			std::stringstream stm;
			stm << "Error 0x" << std::hex << std::setw(8) << std::setfill('0') << dwError << " loading hook DLL.\r\n";
			OutputDebugStringA(stm.str().c_str());
		} else {
			OutputDebugStringA("Hook DLL loaded OK.\r\n");

			procAcceptFrame = reinterpret_cast<AcceptFrame_t>(GetProcAddress(hModHook, "AcceptFrame"));
			if (procAcceptFrame == nullptr) {
				procAcceptFrame = reinterpret_cast<AcceptFrame_t>(GetProcAddress(hModHook, "_AcceptFrame@32"));
				if (procAcceptFrame == nullptr) {
					OutputDebugStringA("Function AcceptFrame not found in hook DLL.\r\n");
					FreeLibrary(hModHook);
					hModHook = NULL;
				}
			}

			procHookInit = reinterpret_cast<HookInit_t>(GetProcAddress(hModHook, "HookInit"));
			if (procHookInit == nullptr) {
				procHookInit = reinterpret_cast<HookInit_t>(GetProcAddress(hModHook, "_HookInit@0"));
				if (procHookInit == nullptr) {
					OutputDebugStringA("Function HookInit not found in hook DLL.\r\n");
					FreeLibrary(hModHook);
					hModHook = NULL;
				} else procHookInit();
			}
	...
	if (procAcceptFrame != nullptr)
		procAcceptFrame(width, height, bpp, pitch, flags, fps, data, pal);
